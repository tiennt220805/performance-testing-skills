...
| 2   | **Enforce Clean & Authorized Baseline** | Require a server restart and/or database re-seed before any comparative test, AND require explicit user confirmation that the target `BASE_URL` is an authorized test/staging environment (never production, never a third-party domain the user doesn't own or control) with any in-scope third-party integrations sandboxed/mocked before a full-load run. Comparing a warm, polluted state against a cold one invalidates the result — and an unauthorized target turns a load test into an unintended denial-of-service attack. |
