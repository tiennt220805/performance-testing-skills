...
6. **Observability Stack File Generation (Optional).** If the user requests a live Prometheus/Grafana dashboard for this engagement, generate `perf-test/observability/docker-compose.yml`, `perf-test/observability/prometheus/prometheus.yml`, and `perf-test/observability/grafana/provisioning/datasources/prometheus.yml` from the templates in `references/observability-stack-setup.md` — the generated files must be immediately runnable, not placeholders. **The agent writes these files and stops there — it MUST NEVER execute `docker compose up` or any other Docker command itself.** Print the exact manual commands from `references/observability-stack-setup.md` §6 for the user to run themselves. This step is entirely skippable — nothing later in the lifecycle depends on it.
